...
if(isset($_POST['save']))
{
	...
	else
	{
		...
		if($count == 0)
			$db->query('INSERT INTO users (name, password, salt, status, dir, rootdir, viewchange, sortord, refchange, viewmode, groupmode, sourcemode, sortkey, refinterval, refmode, detailsstyle, language, design, detailsmode) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
				'sssissisiiiiiiiissi',
				$v_name,
				sha1($v_pass.$salt),
				$salt,
